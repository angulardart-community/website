...
When you're done with this page, the app should look like this {{< exref toh-4 >}}.
...
{{< alert >}}
  ...
  For a multi-word service name, use lower [snake_case]({{< ref glossary >}}#snake_case).
  For example, the filename for `SpecialSuperHeroService` is `special_super_hero_service.dart`.
{{< /alert >}}

Name the class `HeroService`.

{{< excerpt src="lib/src/hero_service_1.dart" section="empty-class" >}}
...
{{< excerpt src="lib/src/hero_service_1.dart" >}}
...
{{< excerpt src="lib/app_component.dart" section="hero-service-import" >}}
...
{{< excerpt src="lib/app_component_1.dart" section="new-service" >}}
...
{{< excerpt src="lib/app_component_1.dart" section="ctor" >}}
...
{{< alert context="info" >}}
  Read more about dependency injection in the [Dependency Injection]({{< ref dependency-injection >}}) page.
{{< /alert >}}

The *injector* doesn't know yet how to create a `HeroService`.
If you run the code now, Angular would fail with an error similar to this:
...
{{< excerpt src="lib/app_component_1.dart" section="providers" >}}
...
{{< excerpt src="lib/app_component_1.dart" section="heroes-and-getHeroes" >}}
...
{{< alert context="info" >}}
  Read more about lifecycle hooks in the [Lifecycle Hooks]({{< ref lifecycle-hooks >}}) page.
{{< /alert >}}
...
{{< excerpt src="lib/app_component_1.dart" section="OnInit-and-ngOnInit" >}}
...
{{< excerpt src="lib/src/hero_service_1.dart" section="getAll" >}}
...
{{< alert >}}
  ...
  [Asynchronous Programming: Futures]: {{< param www >}}/tutorials/language/futures
{{< /alert >}}
...
{{< alert >}}
  ...
  [Declaring async functions]({{< param www >}}/guides/language/language-tour#async) in the Dart language tour.
{{< /alert >}}
...
{{< excerpt src="lib/app_component_1.dart" section="_getHeroes" >}}

Pass the callback function as an argument to the `Future.then()` method:

{{< excerpt src="lib/app_component_2.dart" section="_getHeroes" >}}
...
{{< excerpt src="lib/app_component.dart" section="_getHeroes" >}}

The `Future<void>` return type is the asynchronous equivalent of `void`.

To learn more about using `async` and `await`, read the
[asynchronous programming codelab.]({{< param www >}}/codelabs/async-await)
...
Your app should look like this {{< exref toh-4 >}}.
...
Read about the Angular component router and navigation among the views in the [next tutorial]({{< ref toh-5 >}}) page.
...
{{< excerpt src="lib/src/hero_service.dart" section="getAllSlowly" >}}
...
[dart:async]: {{< param dartApi >}}/stable/dart-async/dart-async-library.html
[Future]: {{< param dartApi >}}/stable/dart-async/Future-class.html
[OnInit]: {{< param pubApi >}}/ngdart/latest/angular/OnInit-class.html
